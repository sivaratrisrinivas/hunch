...
`uv tool install .` puts an isolated `hunch` command on the machine. Other
Python environments keep their packages. `hunch train` fits the transformer
once from Bash history and writes `transformer.pt` under
`$XDG_DATA_HOME/hunch`, or `~/.local/share/hunch`. Closing the terminal does
not delete that file. Train again only when you want a new checkpoint.

`hunch train` prints nothing until it finishes. Then it reports exact-command
accuracy and bits per byte for the transformer, the most-common baseline, and
the command-ngram baseline. Those baselines stay in the report. `predict` still
loads the transformer.

`hunch shell-init` prints Bash code. Running it does not change the calling
shell. One line in `~/.bash_aliases` evaluates that output through the full
path to the installed binary. Interactive Bash already sources that file.
...
inserted. The hook calls `hunch predict`. That command reads the three most
recent usable commands from the history file, loads the checkpoint, and
generates one next command one UTF-8 byte at a time, or it prints nothing.

## Why

A suggestion you have to go fetch will lose to the next thing you type.

`~/.bashrc` sources `~/.bash_aliases` before it prepends `~/.local/bin` to
`PATH`. A bare `hunch` in that file is missing at startup, so the hook never
loads, even though `hunch stats` works later. The aliases line calls
`$HOME/.local/bin/hunch` for that reason.

`Ctrl-R` only finds a line you already typed. The most-common and command-ngram
baselines have the same limit. They pick a command they have counted. The
transformer can emit a line that never appeared whole in history. That is only
useful if you still press Enter to run it, and if a half-typed line is left
alone.

On this CPU a full `hunch predict` often takes more than 200 milliseconds. That
delays the next prompt, so the hook sets `_HUNCH_AUTO=0` for the rest of that
process. Later prompts stay silent. `Ctrl-X Ctrl-P` still requests a
suggestion. A new shell starts with automatic prediction on again.
...
integers. It does not store command context or suggestion text. Exact-command
accuracy asks whether greedy generation matched the next recorded line. Bits
per byte asks how surprised the model is by each next UTF-8 byte. Those are
different questions. The n-gram can win on exact match and still not be what
`predict` loads.

## How

Put this line in `~/.bash_aliases`. Interactive Bash sources that file from
`~/.bashrc`.
...
That call writes the hook to stdout and evaluates it. It appends
...
the length of that text. If `_HUNCH_AUTO` is 0 and nothing is saved, it calls
`hunch predict` first. It never evaluates the suggestion.
...
other two counters. The file mode is `0600`. Train prints after the last
epoch and after greedy evaluation of validation and test. There is no epoch
...
three remaining commands in that file. Commands typed in the current session
are missing until Bash writes them. If the serialized context is longer than
the model's 256-byte window, Hunch keeps the most recent bytes.

It loads the best checkpoint from the private state directory onto the CPU. It
moves that checkpoint to CUDA only if this PyTorch build can actually place it
there. The published install uses CPU wheels, so prediction stays on CPU.
...
from `~/.bashrc`, before `~/.local/bin` is on `PATH`, so the line uses the
installed binary by path.

```bash
eval "$("$HOME/.local/bin/hunch" shell-init)"
```

Open a new shell. A suggestion may print above the prompt. If predict takes
more than 200 milliseconds, later prompts stay silent and `Ctrl-X Ctrl-P`
still inserts on an empty line. It does not run the command.
