...
`hunch predict` prints one next command, or it prints nothing. It reads the
three most recent usable commands from your Bash history, loads the saved
transformer checkpoint, and generates the suggestion one UTF-8 byte at a time.
...
transformer can emit a line that never appeared whole in history, so that is
the model `predict` runs.

Generated bytes can also be truncated, invalid UTF-8, or look like a password.
Predict does not print those.
