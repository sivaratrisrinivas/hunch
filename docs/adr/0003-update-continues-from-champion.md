...
The Scoreboard is the validation split taken at Setup and never re-split. Keep-or-discard uses the transformer's exact-command accuracy on that set only. Those pairs are stored as command text in the state directory so `HISTFILESIZE` cannot move the stick. The Update mix drops those pairs from the old examples so Keep cannot reward rehearsal. The hook counts usable commands with the same predicate Python uses. `hunch stats` plus an update log in that directory show keep versus discard. Command-ngram still takes the Pile and still prints its number. It cannot become the Champion.
